...
def get_bi_inter(n_vertex):
    inter_indices = np.load(abspath+'/neigh_indices/img_indices_'+ str(n_vertex) +'_0.npy')
    inter_weights = np.load(abspath+'/neigh_indices/img_weights_'+ str(n_vertex) +'_0.npy')
...
def singleVertexInterpo_ring(vertex, vertices, tree, neigh_orders, ring_iter=1, ring_threshold=3, threshold=1e-8, debug=False):
    ...
    tmp = (np.linalg.norm(faces_normal, axis=1) < 1e-16).nonzero()[0]
...
#     if k > k_threshold:
#         # print("use nearest neighbor, k=", k)
#         _, top3_near_vertex_index = tree.query(vertex[np.newaxis,:], k=3)
...
def singleVertexInterpo(vertex, vertices, tree, neigh_orders, fast, threshold=1e-8, ring_threshold=3):
    """
...
def multiVertexInterpo(vertexs, vertices, tree, neigh_orders, fast, threshold, ring_threshold):
...
def resampleSphereSurf(vertices_fix, vertices_inter, feat, faces=None, 
                       ...
                       fast=False, threshold=1e-7, ring_threshold=3):
    """
    ...
    neigh_orders : TYPE, optional
        DESCRIPTION. The default is None.
    fast : bool, if using nearest triangle for fast interpolation
    ...
    """ Single process, single thread: 163842: s, 40962: 12.7s, 10242: 3.2s, 2562: 0.8s """
    """ Single process on longleaf: 163842: 20s """
    for i in range(vertices_inter.shape[0]):
        if i % 10000==0:
            print(i)
        inter_indices[i,:] = singleVertexInterpo(vertices_inter[i,:], vertices_fix, tree, neigh_orders, fast, threshold=1e-8, ring_threshold=3)
...
#                                              vertices_fix, tree, neigh_orders, fast, threshold, ring_threshold)))
#
#    pool.close()
#    pool.join()
#
